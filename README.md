...
- Система не выдумывает ответы, а выдает заранее описанный ответ
...
‼️*[Скриншот-кандидат: пример полного диалога бронирования — от запроса до подтверждения]*
...
│
▼
Родительское приложение (этот репозиторий)
│
├── Regulation-вопрос → HTTP-вызов → RAG-приложение (дочернее, документация отдельно)
│
└── Booking-сценарии (reserve / my_bookings / cancel) — остаются в родительском
│
▼
...
‼️*[Схема-кандидат: диаграмма родитель→RAG-дочернее приложение, показывающая границу HTTP-вызова]*
...
‼️ Подробная схема rag-подсистемы — в [Architecture Overview](docs/architecture-overview.md).

‼️ Подробная схема booking-подсистемы — в [Architecture Overview](docs/architecture-overview.md).
---

## Подходы и методики, реализованные в этом проекте

Короткая расшифровка терминов, что каждый из них означает и где именно в проекте реализован. Полная версия с подробными объяснениями и перекрёстными ссылками на ADR — в [Glossary](docs/glossary.md).
...
‼️ *[Скриншот-кандидат: граф pipeline в Dify — общий вид с ветками reserve/my_bookings/cancel]*

---

## Архитектурные паттерны, применённые в проекте

Короткая расшифровка — что означает каждый термин и где он реализован. Подробный глоссарий с объяснениями «для непосвящённых» — в [Glossary](docs/glossary.md).
...
| Документ | Что внутри |
|----------|------------|
| ‼️`project-overview.md` | Что это, зачем, ключевые сценарии |
| ‼️`architecture-overview.md` | Полная техническая схема — БД, RPC-слой, pipeline, управление состоянием |
| ‼️`adr-log.md` | История архитектурных решений — что рассматривалось, что выбрано и почему (включая отклонённые варианты) |
| ‼️`api-contract.md` | Контракт всех RPC-функций Supabase — параметры, edge cases, примеры |
| ‼️`known-limitations-roadmap.md` | Что осознанно не реализовано и почему |
| ‼️`test-plan.md` | Свод тестовых сценариев с ожидаемыми результатами |
| ‼️`glossary.md` | Подробный глоссарий терминов с перекрёстными ссылками на ADR |
...
‼️ *[Скриншот-кандидат: фрагмент ADR-лога с 2-3 записями]*
